...
def train_model(
	...
	concat_noisynormdrop_one = Concatenate() >> BatchNorm() >> Dropout(proba=.2) >> GaussianNoise(std=0.2)
	concat_noisynormdrop_two = Concatenate() >> BatchNorm() >> Dropout(proba=.2) >> GaussianNoise(std=0.2)
