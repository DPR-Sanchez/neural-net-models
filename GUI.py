...
async def gui_layouts(layout):
	...
	if layout == 'general':
		...
		general_training_layout = [
			...
			[sg.Text('training accuracy: ', size=(30, 7), key='accuracy'), sg.Text('training dataset: ', size=(30, 1),key='training dataset')],
				...
				if event is None or event == 'Exit':
					...
					if values[6] != 'Select dataset >>' and values[6][-3:] == 'csv':
						...
						if all([type(1) == type(x) for x in [values[2], values[3], values[4]]]):
							...
							optimizer,trained_net,accuracy = models_training.train_model(
																							numpy_seed=values[2],
																							tensor_seed=values[3],
																							ran_seed=values[4],
					...
					if values[6] != 'Select dataset >>' and values[6][-3:] == 'csv':
						if trained_net is not None:
							accuracy,actual,predicted,delta,miss,zero_one_accuracy,bounds = models_training.prediction(
																	trained_net,
																	mode='accuracy',
																	data_source=values[6],
																	index=values[7],
							...
							general_training_window.Element('accuracy').Update(
																					...
																					f'0/1 delta: {delta} \n' \
																					f'0/1 miss: {miss} \n' \
																					f'0/1 accuracy: {zero_one_accuracy} \n' \
